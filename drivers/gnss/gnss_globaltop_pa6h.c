				       ...
				       void *user_data)
{
	...
	if (!gnss_nmea0183_validate_message(argv, argc)) {
		return;
	}

	if (gnss_parse_atoi(argv[2], 10, &flag) < 0) {
		return;
	}
...
static int globaltop_pa6h_set_enabled_systems(const struct device *dev, gnss_systems_t systems)
{
	...
	data->enabled_systems |= (systems & GNSS_SYSTEM_SBAS);
...
#define GLOBALTOP_PA6H(inst)                                                                       \
	...
	BUILD_ASSERT((DT_INST_PROP(inst, fix_interval_ms) >= 100) &&                               \
		     ...
		     "Invalid fix-rate. Please set it between 100 and 10000 ms");                  \
                                                                                                   \
